...
$breadcrumb[1] = array(
"NAME" => $ar_res['NAME'],
"URL" => $breadcrumb[0]["URL"].$arParams["~SECTION_CODE_PATH"]."/",
);  


//Вид оборудования в крошка


$namevid = $arResult[ITEMS][153][NAME];


$qqq112 = $arResult[ITEMS][153][CODE];


foreach ($arResult[ITEMS][153][VALUES] as $val => $ar1){
	if ($ar1["CHECKED"]){
		 $urlval =  "filter/".$qqq112."-is-".$ar1[URL_ID]."/apply/";
		 
		 $breadcrumb[2] = array(
			"NAME" => $ar1[VALUE],
			"URL" => $breadcrumb[1]["URL"].$urlval,
			); 
...
$_SESSION['breadcr'] = $breadcrumb;


?>


<div class="becr" style="display:none;">
...
<?
				...
				echo "</pre><br><br>";
				*/
				//$valfilt = "$ar1[VALUE] ($ar1[ELEMENT_COUNT])";
				//echo $valfilt ;
...
<script>
function tt(q,w){//тест
	$(q).parents(".sectionCatalogFilter-selected").children(".sectionCatalogFilter-selected__value").text(w);
}
</script>
...
//Вывод выбранного товара НЕОБХОДИМО ДЛЯ ОПРЕДЕЛЕНИЯ КОЛИЧЕСТВА ТОВАРОВ
...
$filter = array(
	...
	$filter = array(
    "IBLOCK_ID" => $arParams["IBLOCK_ID"],
    "SECTION_ID" => $arParams[SECTION_ID] // если есть
);
...
if($arParams[SECTION_ID]==0){
	$filter2 = array("IBLOCK_ID" => $arParams["IBLOCK_ID"]);
}else{
	$filter2 = array("IBLOCK_ID" => $arParams["IBLOCK_ID"],"SECTION_ID" => $arParams[SECTION_ID]);
}

//Раздел для все
/*
$value_to_delete = '=PROPERTY_125'; 
$arrFilter11 = $arrFilter;
unset($arrFilter11[$value_to_delete]);
$filter11 = array_merge($filter2, $arrFilter11);
$cnt11 = CIBlockElement::GetList(
    array(),
    array($filter11),
    array(),
    false,
...
?>
                ...
                <div class="sectionCatalogFilter">
                  <div class="sectionCatalogFilter__wrap">
                    <div class="sectionCatalogFilter__row one filt1 <?=(current($arrFilter)[0] ? "visable1" : "unvisable1");?>">
					...
					?>
						...
						foreach ($arItem["VALUES"] as $val => $ar1){
							//$class1 = "";
							if ($ar1["CHECKED"]){
								$class1 = "";
								$valfilt = "$ar1[VALUE] ($ar1[ELEMENT_COUNT])";
						...
						<div class="sectionCatalogFilter-selected">
							...
							<div class="sectionCatalogFilter-selected__value">
							
							<?=($valfilt == "" ? $arItem["NAME"] : $valfilt);?>
                        ...
                        <div class="sectionCatalogFilter-selected__items">
                          ...
                          <div class="sectionCatalogFilter-selected__item <?=$class1; ?>"
						  
						  onclick="
						$('#<?=CUtil::JSEscape("all_".$arCur["CONTROL_ID"])?>').prop('checked', true);  
						smartFilter.selectDropDownItem(this, '<?=CUtil::JSEscape("all_".$arCur["CONTROL_ID"])?>');">
						  
						  
						  
						  
						  
                            <div class="sectionCatalogFilter-selected__text"><?=GetMessage("CT_BCSF_FILTER_ALL"); ?></div>
                            <div class="sectionCatalogFilter-selected__count">
							
							
							<?
								//Раздел для все
								$value_to_delete = '=PROPERTY_'.$arItem[ID]; 
								$arrFilter11 = $arrFilter;
								unset($arrFilter11[$value_to_delete]);
								$filter11 = array_merge($filter2, $arrFilter11);
								$cnt11 = CIBlockElement::GetList(
						...
						<?//Сортировка значений по количеству ?>
						
						<?
							$valsort = $arItem["VALUES"];
							usort($valsort, function($a, $b){
						...
						?>
							  ...
							  <div class="sectionCatalogFilter-selected__item <?=$class;?>" 
							  onclick="
							$('#<?=CUtil::JSEscape($ar["CONTROL_ID"])?>').prop('checked', true);
							smartFilter.selectDropDownItem(this, '<?=CUtil::JSEscape($ar["CONTROL_ID"])?>');
							">
								<div class="sectionCatalogFilter-selected__text"><?=$ar["VALUE"]?></div>
								<div class="sectionCatalogFilter-selected__count"><? echo $ar["ELEMENT_COUNT"]; ?></div>
							  </div>
						...
						?>
							  ...
							  <div class="sectionCatalogFilter-selected__item <?=$class;?> <? if($ar[ELEMENT_COUNT]==0) echo 'noneclick'?>" 
							  
							  style="<? if($ar[ELEMENT_COUNT]==0) echo 'color: #b7b7b7 !important;'?>"
							  
							  onclick="
							  <? if($ar[ELEMENT_COUNT]!=0){?>
							  
							  tt(this, '<?=$ar["VALUE"]?> (<? echo $ar["ELEMENT_COUNT"]; ?>)');
					...
					?>  
					  ...
					  <?//Тест сбросить все?>  
					<div class="sectionCatalogFilter-info__col two mobifilter" onclick="
						<? foreach($arResult["ITEMS"] as $key=>$arItem){
								if (empty($arItem["VALUES"]) || isset($arItem["PRICE"]))
									continue;
								if ($arItem["DISPLAY_TYPE"] == "A" && ( $arItem["VALUES"]["MAX"]["VALUE"] - $arItem["VALUES"]["MIN"]["VALUE"] <= 0))
                    ...
                    <div class="sectionCatalogFilter__row two">
                      <div class="sectionCatalogFilter-info">
                        ...
                        <div class="sectionCatalogFilter-info__col one">
                          ...
                          <h2 class="sectionCatalogFilter-info__title">Фильтр товаров <span>(<?=$cnt;?>)</span></h2>
                        </div>
                        ...
                        <div class="sectionCatalogFilter-info__col two" onclick="
							<? foreach($arResult["ITEMS"] as $key=>$arItem){
									if (empty($arItem["VALUES"]) || isset($arItem["PRICE"]))
										continue;
									if ($arItem["DISPLAY_TYPE"] == "A" && ( $arItem["VALUES"]["MAX"]["VALUE"] - $arItem["VALUES"]["MIN"]["VALUE"] <= 0))
...
//echo "test";//.$arParams[SECTION_ID];


?>


<div class="smart-filter mb-4 <?=$templateData["TEMPLATE_CLASS"]?> <?if ($arParams["FILTER_VIEW_MODE"] == "HORIZONTAL") echo "smart-filter-horizontal"?>" style="display:none;">
								...
								?>
									...
									<div class="smart-filter-input-group-dropdown">
										<div class="smart-filter-dropdown-block" onclick="smartFilter.showDropDownPopup(this, '<?=CUtil::JSEscape($key)?>')">
												...
												<ul>
													<li>
														<label for="<?="all_".$arCur["CONTROL_ID"]?>"
															   class="<?=CUtil::JSEscape("all_".$arCur["CONTROL_ID"])?>  smart-filter-dropdown-label"
													...
													?>
														<li>
															...
															<label  class="<?=$ar["CONTROL_ID"]?>"  id="<?=$ar["CONTROL_ID"]?>" for="<?=$ar["CONTROL_ID"]?>"
																   class="smart-filter-dropdown-label<?=$class?> <?=$ar["CONTROL_ID"]?>"
											...
											<div  onclick="$('.<?=$ar["CONTROL_ID"]?>').trigger('click');">
											...
											data-role="label_<?=$ar["CONTROL_ID"]?>"
											for="<?=$ar["CONTROL_ID"]?>"
											class="ttt1 smart-filter-dropdown-label<?=$class?> <?=$ar["CONTROL_ID"]?>"

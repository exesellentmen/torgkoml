...
<script> 
//Функция замены адресной строки
/*
var stateParameters = { foo: "bar" };
var uri = "/category/tech/page/2"; // или предварительно формирование uri
				...
				foreach($arResult["ITEMS"] as $key=>$arItem)
				{
					...
					<div class="sectionCatalogFilter-selected">
						...
						<div class="sectionCatalogFilter-selected__items">
							...
							<div class="sectionCatalogFilter-selected__item <?=$classall?>"  for="<?="all_".$arCur["CONTROL_ID"]?>" data-role="label_<?="all_".$arCur["CONTROL_ID"]?>" onclick="$('#<?=CUtil::JSEscape("all_".$arCur["CONTROL_ID"])?>').prop('checked', true); smartFilter.selectDropDownItem(this, '<?=CUtil::JSEscape("all_".$arCur["CONTROL_ID"])?>')">					   
							...
							<?foreach ($arItem["VALUES"] as $val => $ar):
								...
								if ($ar["DISABLED"])
									$class.= " ";
							?>
								<div class="sectionCatalogFilter-selected__item <?=$class;?>"  for="<?=$ar["CONTROL_ID"]?>" data-role="label_<?=$ar["CONTROL_ID"]?>" onclick="
      ...
      <div class="sectionCatalogFilter-selected">
        ...
        <div class="sectionCatalogFilter-selected__items">
          ...
          <div class="sectionCatalogFilter-selected__item active">
            <div class="sectionCatalogFilter-selected__text">Все</div>
...
{?>
	...
	<div class="sectionCatalogFilter-selected">
        ...
        <div class="sectionCatalogFilter-selected__items">
			...
			<div class="sectionCatalogFilter-selected__item active"  onclick="
			$('#<?=CUtil::JSEscape("all_".$arCur["CONTROL_ID"])?>').prop('checked', true);
			$('.<?=CUtil::JSEscape("all_".$arCur["CONTROL_ID"])?>').trigger('click');">
				<div class="sectionCatalogFilter-selected__text"><?=GetMessage("CT_BCSF_FILTER_ALL"); ?></div>
				<div class="sectionCatalogFilter-selected__count">213</div>
			</div>
	...
	?>
		...
		<div class="sectionCatalogFilter-selected__item"  onclick="$('#<?=CUtil::JSEscape($ar["CONTROL_ID"])?>').prop('checked', true); $('.<?=$ar["CONTROL_ID"]?>').trigger('click');">
            <div class="sectionCatalogFilter-selected__text"><?=$ar["VALUE"]?></div>
            <div class="sectionCatalogFilter-selected__count">
				<? if ($arParams["DISPLAY_ELEMENT_COUNT"] !== "N" && isset($ar["ELEMENT_COUNT"])):?>
					<? echo $ar["ELEMENT_COUNT"]; ?>
                ...
                <div class="sectionCatalogFilter">
                  <div class="sectionCatalogFilter__wrap">
                    <div class="sectionCatalogFilter__row one filt1 unvisable1">
                      ...
                      <div class="sectionCatalogFilter-selected" onclick="
	...
	<div class="smart-filter-section">
		...
		<form name="<?echo $arResult["FILTER_NAME"]."_form"?>" action="<?echo $arResult["FORM_ACTION"]?>" method="get" class="smart-filter-form">
			...
			<div class="row">
				...
				<?
				/*Замена адресной строки при ajax*/
				$urr="";
				foreach($arResult["ITEMS"] as $key=>$arItem1)
				{
					foreach ($arItem1["VALUES"] as $val => $ar1)
					{
						if ($ar1["CHECKED"])
						{
							$checkedItemExist = true;
							$urr=$urr."/".$arItem1[CODE]."-is-".$ar1[URL_ID];
						}
					}
				}
				?>
				<script>
					var stateParameters = { foo: "bar" };
					var uri = "<?=$urr; ?>";
					var uri = "<?=str_replace("/clear", "", $arResult[JS_FILTER_PARAMS][SEF_DEL_FILTER_URL]).$urr; ?>";
					history.pushState(stateParameters, "New page title", uri);
					history.pathname = uri;
				</script>
									...
									<div class="smart-filter-input-group-dropdown">
										<div class="smart-filter-dropdown-block" onclick="smartFilter.showDropDownPopup(this, '<?=CUtil::JSEscape($key)?>')">
												...
												<ul>
													...
													?>
														<li>
															...
															<label style="/*display:none;*/" class="<?=$ar["CONTROL_ID"]?>"  id="<?=$ar["CONTROL_ID"]?>" for="<?=$ar["CONTROL_ID"]?>"
											...
											<?/*
												...
												<div onclick="
												$('.<?=$ar["CONTROL_ID"]?>').css('background-color','aqua');
												
												$('#<?=CUtil::JSEscape($ar["CONTROL_ID"])?>').prop('checked', true); 
												
												$('.<?=$ar["CONTROL_ID"]?>').trigger('click');
												alert('<?=$ar["CONTROL_ID"];?>');" >
...
<div onclick="
$('.arrFilter_125_1120914852').css('background-color','aqua');

$('.arrFilter_125_1120914852').trigger('click');

...
echo $url;
				*/
				...
				//Если торговые системы
				/*
				if($arResult[SECTION][ID] == 67){
				//	echo "Торговые системы";
					
					
					//Определение всех вложенных секций
					
					$arr_sectioonid = array();
					
					foreach ($arResult['SECTIONS'] as &$arSection)
					{
						
						array_push($arr_sectioonid, $arSection['ID']);
					...
					$arFilter = Array('IBLOCK_ID'=>$IBLOCK_ID, 'GLOBAL_ACTIVE'=>'Y', 'SECTION_ID' => $arr_sectioonid);
					...
					while ($arSect = $db_list->GetNext())
					   {
						...
						//   echo $arSect[NAME]."<br>";
						//  echo CFile::GetPath($arSect[PICTURE])."<br>";
						...
						if (false === $arSect['PICTURE'])
							$arSect['PICTURE'] = array(
								'SRC' => $arCurView['EMPTY_IMG'],
								'ALT' => (
									'' != $arSect["IPROPERTY_VALUES"]["SECTION_PICTURE_FILE_ALT"]
									? $arSect["IPROPERTY_VALUES"]["SECTION_PICTURE_FILE_ALT"]
									: $arSect["NAME"]
								),
								'TITLE' => (
									'' != $arSect["IPROPERTY_VALUES"]["SECTION_PICTURE_FILE_TITLE"]
									? $arSect["IPROPERTY_VALUES"]["SECTION_PICTURE_FILE_TITLE"]
									: $arSect["NAME"]
								)
							);
							?>
							<li id="<? echo $this->GetEditAreaId($arSect['ID']); ?>"  class="col-6 col-sm-3 col-md-2 catalog-section-list-item">
								<div class="catalog-section-list-tile-img-container">
									<a
										href="<? echo $arSect['SECTION_PAGE_URL']; ?>"
										class="catalog-section-list-item-img"
										style="background-image:url('<?=CFile::GetPath($arSect[PICTURE]); ?>');"
										title="<? echo $arSect['PICTURE']['TITLE']; ?>"
										></a>
								</div>
								<? if ('Y' != $arParams['HIDE_SECTION_NAME'])
								{
									?>
										<div class="catalog-section-list-item-inner">
											<h3 class="catalog-section-list-item-title">
												<a class="catalog-section-list-item-link" href="<? echo $arSect['SECTION_PAGE_URL']; ?>">
													<? echo $arSect['NAME']; ?>
												</a>
												<? if ($arParams["COUNT_ELEMENTS"])
												{
													?>
													<span class="catalog-section-list-item-counter">(<? echo $arSect['ELEMENT_CNT']; ?>)</span>
													<?
												}
												?>
											</h3>

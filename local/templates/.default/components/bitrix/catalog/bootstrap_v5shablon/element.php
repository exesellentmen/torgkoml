...
//Если вывели окно а не перешли по ссылки
$query = $APPLICATION->sUriParam;
$nonehistory = true; // Переменная в которую помещаем проверку на использование навигации в браузере
...
foreach (explode('&', $query) as $chunk) {
    ...
    if ($param) {
		...
		if((urldecode($param[0]) == "test")&&(urldecode($param[1]) == "y"))
			$nonehistory = false;
...
$component->arParams[AJAX_OPTION_HISTORY] = "N";
*/
...
require($_SERVER["DOCUMENT_ROOT"]."/local/templates/eshop_bootstrap_v4/header.php");
...
/*		
		echo "tes= ".$arParams['DETAIL_META_DESCRIPTION']."<br>";
		echo "tes= ".$arParams['DETAIL_BROWSER_TITLE']."<br>";
		echo "tes= ".$arParams['DETAIL_SET_CANONICAL_URL']."<br>";
*/
...
//		$componentElementParams['AJAX_OPTION_HISTORY'] = "N";
	...
	//	echo 1;
		...
		/*
		echo "<pre>";
		print_r($componentElementParams);
		print_r($component);
		echo "</pre>";
		*/

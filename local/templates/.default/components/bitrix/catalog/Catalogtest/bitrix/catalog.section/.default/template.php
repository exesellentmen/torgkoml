...
//foreach($arResult[NAV_RESULT]->arResult as $item){
	...
	echo "<br><br>$item[NAME] $item[ID]";
	echo "<br>Торговые предложения: <br>";
		...
		// Назначение фильтра по инф блоку, по id категории
		 $arFilter1    = Array(
		  ...
		  $res = CIBlockElement::GetList(Array(), $arFilter1 , false);
			...
			//	echo "<input name='buy' type='button' value='".GetMessage('CATALOG_BUY')."' OnClick='window.location='".CUtil::JSEscape($arElement['DETAIL_PAGE_URL'].'#buy')."'' />";
			//	echo "<br>";

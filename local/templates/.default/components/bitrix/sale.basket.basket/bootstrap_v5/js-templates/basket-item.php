...
{{^SHOW_RESTORE}}
	...
	<div class="cartStep1-table__tr" id="basket-item-{{ID}}" data-entity="basket-item" data-id="{{ID}}">
		<div class="cartStep1-table__td one"><a class="cartStep1-table__pic" href="{{#DETAIL_PAGE_URL}}{{DETAIL_PAGE_URL}}{{/DETAIL_PAGE_URL}}"><img class="cartStep1-table__img" src="{{{IMGITEM}}}{{^IMGITEM}}<?= $templateFolder ?>/images/no_photo.png{{/IMGITEM}}" alt=""></a></div>
		<div class="cartStep1-table__td two">
			<div class="cartStep1-table__info">
				<a class="cartStep1-table__link" href="{{#DETAIL_PAGE_URL}}{{DETAIL_PAGE_URL}}{{/DETAIL_PAGE_URL}}">{{NAME}}</a>

				<span class="cartStep1-table__delete delmobi" style="    cursor: pointer;">
					<div class="lnr lnr-trash toggleTitle" data-title="Удалить" data-entity="basket-item-delete"></div>
				</span>

				<div class="cartStep1-table__art">Артикул: {{#ARTICLE}} {{ARTICLE}} {{/ARTICLE}}</div>
			</div>
		</div>
		<div class="cartStep1-table__td three">
			<div class="cartStep1-table-price-wrap">
				...
				{{#SHOW_DISCOUNT_PRICE}}
					<div class="cartStep1-table__price">{{{PRICE_FORMATED}}}</div>
				{{/SHOW_DISCOUNT_PRICE}}

				{{^SHOW_DISCOUNT_PRICE}}
					<div class="cartStep1-table__price" style="color: #2f2f2f;">{{{PRICE_FORMATED}}}</div>
				{{/SHOW_DISCOUNT_PRICE}}

				{{#SHOW_DISCOUNT_PRICE}}
					<div class="cartStep1-table__price-old">{{{FULL_PRICE_FORMATED}}}</div>
				{{/SHOW_DISCOUNT_PRICE}}
		...
		<?//Вывод количестка и кнопок плюс и минус?>
		<div class="cartStep1-table__td four">
			<div class="cartStep1-table-count" data-entity="basket-item-quantity-block">

				<div class="cartStep1-table-count__col one">
					<button class="cartStep1-table-count__btn1 custp" data-entity="basket-item-quantity-minus">-</button>
				</div>

				<div class="cartStep1-table-count__col two">
					<div class="cartStep1-table-count__input">
						<input class="cartStep1-table-count__input-text" type="text" value="{{QUANTITY}}" {{#NOT_AVAILABLE}} disabled="disabled" {{/NOT_AVAILABLE}} data-value="{{QUANTITY}}" data-entity="basket-item-quantity-field" id="basket-item-quantity-{{ID}}">
					</div>
				</div>

				<div class="cartStep1-table-count__col three">
					<button class="cartStep1-table-count__btn1 custp" data-entity="basket-item-quantity-plus">+</button>
				</div>
		...
		<?//Вывод суммы цены?>
		<div class="cartStep1-table__td five">


			<span class="cartStep1-table__delete deldesk" style="    cursor: pointer;">
				<div class="lnr lnr-trash toggleTitle" data-title="Удалить" data-entity="basket-item-delete"></div>
			</span>
...
*/?>
			...
			{{#SHOW_DISCOUNT_PRICE}}
				<div class="cartStep1-table__total sale" id="basket-item-sum-price-{{ID}}" data-total="{{#PRISEOLD}}{{PRISEOLD}}{{/PRISEOLD}}">
					{{{SUM_PRICE_FORMATED}}}
				</div>
			{{/SHOW_DISCOUNT_PRICE}}

			{{^SHOW_DISCOUNT_PRICE}}
				<div class="cartStep1-table__total sale" id="basket-item-sum-price-{{ID}}" data-total="{{#PRISEOLD}}{{PRISEOLD}}{{/PRISEOLD}}" style="color: #2f2f2f;">
					{{{SUM_PRICE_FORMATED}}}
				</div>
			{{/SHOW_DISCOUNT_PRICE}}
			...
			{{#SHOW_DISCOUNT_PRICE}}
				<div class="cartStep1-table__total-old sale" data-sale="{{#PRISENEW}}{{PRISENEW}}{{/PRISENEW}}">{{#SHOW_DISCOUNT_PRICE}} {{{SUM_FULL_PRICE_FORMATED}}} {{/SHOW_DISCOUNT_PRICE}}</div>
			{{/SHOW_DISCOUNT_PRICE}}
	...
	<div class="basket-items-list-item-container{{#SHOW_RESTORE}} basket-items-list-item-container-expend{{/SHOW_RESTORE}}" id="basket-item-{{ID}}" data-entity="basket-item" data-id="{{ID}}">
		...
		Артикуль {{#ARTICLE}} {{ARTICLE}} {{/ARTICLE}} <br>
		{{#DISCOUNTCUST}}Скидка {{DISCOUNTCUST}} <br>{{/DISCOUNTCUST}}
		{{#SHOW_DISCOUNT_PRICE}} Старая цена {{{FULL_PRICE_FORMATED}}} <br>{{/SHOW_DISCOUNT_PRICE}}
		Новая цена {{{PRICE_FORMATED}}}<br><br>
		Количество: <br>
	...
	Цена со скидкой Сумма {{{SUM_PRICE_FORMATED}}}<br>



	{{#SHOW_DISCOUNT_PRICE}}
		Сумма Старая цена: {{{SUM_FULL_PRICE_FORMATED}}}<br>
	{{/SHOW_DISCOUNT_PRICE}}
...
?>
	...
	Ссылка на изобрежение<br>
	{{{IMAGE_URL}}}{{^IMAGE_URL}}<?= $templateFolder ?>/images/no_photo.png{{/IMAGE_URL}}<br>

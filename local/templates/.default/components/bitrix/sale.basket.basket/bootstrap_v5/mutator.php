...
$imgt = "";
if(strpos($row[PROPERTY_Gallery_VALUE], ',')){
	$imgt = substr($row[PROPERTY_Gallery_VALUE], 0,strpos($row[PROPERTY_Gallery_VALUE], ','));
}else
$imgt = $row[PROPERTY_Gallery_VALUE];
$imgt = CFile::GetPath($imgt);
	...
	$rowData = array(
		...
		'DISCOUNTCUST' => ($row['DISCOUNT_PRICE_PERCENT_FORMATED'] == "0%" ? "" : $row['DISCOUNT_PRICE_PERCENT_FORMATED'] ),
		'PRISEOLD' => round($row[PRICE]),
		'PRISENEW' => round($row[BASE_PRICE]),
...
$wwq = 0;
foreach ($result[GRID][ROWS] as $key => $value1) {
	$wwq += $value1[QUANTITY];
}
